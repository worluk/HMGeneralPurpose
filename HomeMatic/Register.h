...
extern uint32_t peerdb[maxChannel][maxPeer];

const uint8_t peermax[] = {6,6,6};

//- -----------------------------------------------------------------------------------------------------------------------
// - Channel device config ------------------------------------------------------------------------------------------------
typedef struct s_peer_regChan_actor_ {                 // chn:1, lst:3
...
} s_peer_regChan_actor;
typedef struct s_peer_regChan_remote_ {                 // chn:6, lst:4
	uint8_t peerNeedsBurst      :1; // reg:0x01, sReg:1
	uint8_t                     :6;
	uint8_t expectAES           :1; // reg:0x01, sReg:1.7
} s_peer_regChan_remote;
typedef struct s_dev_regChan_actor_ {
	uint8_t sign                :1; // reg:0x08, sReg:8
} s_dev_regChan_actor;
typedef struct s_dev_regChan_remote_ {
	uint8_t                     :4;
	uint8_t longPress           :4; // reg:0x04, sReg:4.4
	uint8_t sign                :1; // reg:0x08, sReg:8
	uint8_t                     :7;
	uint8_t dblPress            :4; // reg:0x09, sReg:9
} s_dev_regChan_remote;
typedef struct s_regChan_actor_ {
	s_dev_regChan_actor  list1;
	s_peer_regChan_actor peer[6];
} s_regChan_actor;
typedef struct s_regChan_remote_ {
...
typedef struct s_cpy_regChan_actor_ {
	s_dev_regChan_actor  l1;
	s_peer_regChan_actor l3;
} s_cpy_regChan_actor;

typedef struct s_cpy_regChan_remote_ {
	s_dev_regChan_remote  l1;
	s_peer_regChan_remote l4;
} s_cpy_regChan_remote;

typedef struct s_regCpy_ {
	s_regDev    ch0;
